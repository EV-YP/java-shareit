...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemWithDatesDto> getItemsByOwner(Long ownerId) {
        ...
        return items.stream().map(item -> {
            ...
            dto.setNextBooking(getLocalDateTimeOrNull(nextInstant));
            Instant lastInstant = lastBookingEnds.get(item.getId());
            dto.setLastBooking(getLocalDateTimeOrNull(lastInstant));
    ...
    public CommentDto addComment(Long userId, Long itemId, CommentDto commentDto) {
        ...
        Item item = itemRepository.findById(itemId)
                .orElseThrow(() -> new NotFoundException(String.format("Предмет с id: %d не найден", itemId)));
        boolean hasPastBooking = bookingRepository.existsByItemIdAndBookerIdAndStatusAndEndBefore(itemId, userId, APPROVED, Instant.now());
        if (!hasPastBooking) {
